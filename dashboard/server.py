...
# Fixed protocol section — NEVER generated by LLM, always appended verbatim
_SOUL_PROTOCOL = """
## System Commands (MANDATORY — use these exact formats)

### Communication
- `@AgentName instruction` — delegate task to a teammate
- `@Master report` — report results to human master
- Multiple mentions OK: `@CMO task1 @CTO task2`

### Task Management
- `[TASK_ADD:name:high/medium/low]` — create kanban task
- `[TASK_START:name]` — mark as in-progress
- `[TASK_DONE:name]` — mark as completed
- `[TASK_BLOCK:name:reason]` — flag as blocked

### Approvals
- `[APPROVAL:category:title:detail]` — submit to master for decision
  Categories: budget, purchase, project, hr, policy, general
- Use when: master judgment needed, real-world action required, budget issues
...
- Execute tasks IMMEDIATELY. No prep talk ("I'll check..." is forbidden).
- Every response MUST contain @mention OR system command.
- Report RESULTS, not intentions.
- NO_REPLY is forbidden.
- Save deliverables to `_shared/deliverables/`
- No deadlines. Everything is ASAP.
"""
            ...
            if result and len(result) > 30:
                ...
                if soul_file.exists():
                    ...
                    if '## System Commands' not in existing:
                        soul_file.write_text(existing + _SOUL_PROTOCOL, encoding='utf-8')
