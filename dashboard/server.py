...
# Fixed protocol section — command SYNTAX is always English, descriptions are localized
_PROTOCOL_FILE = BASE / "dashboard" / "i18n" / "protocol.json"

def _load_protocol_strings(lang='en') -> dict:
    try:
...
def _build_soul_protocol(lang='en') -> str:
    """Build the fixed protocol section with localized descriptions.
    Command syntax stays English; explanations use the selected language."""
    p = _load_protocol_strings(lang)
    if not p:
        p = _load_protocol_strings('en')
    return f"""
## {p.get('system_commands', 'System Commands (MANDATORY)')}

### {p.get('communication', 'Communication')}
- `@AgentName instruction` — {p.get('comm_delegate', 'delegate task to a teammate')}
- `@Master report` — {p.get('comm_report', 'report results to human master')}
- {p.get('comm_multiple', 'Multiple mentions OK')}: `@CMO task1 @CTO task2`

### {p.get('task_mgmt', 'Task Management')}
- `[TASK_ADD:name:high/medium/low]` — {p.get('task_add', 'create kanban task')}
- `[TASK_START:name]` — {p.get('task_start', 'mark as in-progress')}
- `[TASK_DONE:name]` — {p.get('task_done', 'mark as completed')}
- `[TASK_BLOCK:name:reason]` — {p.get('task_block', 'flag as blocked')}

### {p.get('approvals', 'Approvals')}
- `[APPROVAL:category:title:detail]` — {p.get('approval_submit', 'submit to master for decision')}
  {p.get('approval_categories', 'Categories: budget, purchase, project, hr, policy, general')}
- {p.get('approval_when', 'Use when: master judgment needed, real-world action required, budget issues')}

### {p.get('scheduling', 'Scheduling')}
...
- {p.get('rule_results', 'Report RESULTS, not intentions.')}
- {p.get('rule_no_reply', 'NO_REPLY is forbidden.')}
- {p.get('rule_deliverables', 'Save deliverables to _shared/deliverables/')}
- {p.get('rule_asap', 'No deadlines. Everything is ASAP.')}
"""
...
def _generate_custom_soul(agent_id, workspace, name, role, company_name, topic, lang):
    ...
    def _gen():
        prompt = (
            ...
            f"Write in {lang_name}. Include ONLY these sections:\n"
            f"1. ## Identity — who they are, their expertise at THIS company\n"
            f"2. ## Skills — specific skills and focus areas for this role at this company\n"
            f"3. ## Deliverables — what artifacts they should produce\n"
            f"4. ## Communication — who they report to, who they delegate to\n\n"
            f"Do NOT include system commands or rules (those are added separately).\n"
            f"Return ONLY markdown (no ```). Keep it under 25 lines."
        )
        try:
...
async def api_i18n_generate(request: Request):
    ...
    # English protocol descriptions (for translation)
    protocol_en = _load_protocol_strings('en')
    ...
    def _gen():
        try:
            ...
            # Merge protocol into protocol.json
            if protocol_tpl:
                try:
                    all_proto = json.loads(_PROTOCOL_FILE.read_text(encoding='utf-8'))
                except (OSError, json.JSONDecodeError):
                    all_proto = {}
                # Only keep keys that exist in English version
                en_keys = set(_load_protocol_strings('en').keys())
                clean_proto = {k: v for k, v in protocol_tpl.items() if k in en_keys and isinstance(v, str) and v.strip()}
                if clean_proto:
                    all_proto[lang_code] = clean_proto
                    _PROTOCOL_FILE.write_text(json.dumps(all_proto, indent=2, ensure_ascii=False), encoding='utf-8')

            print(f"[i18n] Generated {lang_code}: ui={len(ui_strings)} roles={len(roles_map)} welcome={len(welcome_tpl)} protocol={len(protocol_tpl)}")
            return {
                'ok': True,
                'lang_code': lang_code,
                'keys': len(ui_strings),
